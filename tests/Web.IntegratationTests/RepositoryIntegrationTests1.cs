...
{
  public class RepositoryIntegrationTests1(CustomWebApplicationFactory<Program> factory) : IClassFixture<CustomWebApplicationFactory<Program>>, IAsyncLifetime
  {
      public async Task InitializeAsync()
        {
            ...
            await SeedData1.PopulateTestData(context);
        }
        ...
        public async Task GetAllListIncluding_ShouldReturnEntitiesWithIncludedProperties()
        {
            ...
            {
                var context = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
                var repository = new EfRepository<Person, int>(context);

                var entities = await repository.GetAllIncluding(p => p.Tasks).ToListAsync();

                Assert.NotNull(entities);
                var firstPerson = entities.FirstOrDefault(p => p.Id == 1);
        ...
        public async Task GetAllListIncluding_ShouldReturnEntitiesWithOutProperties()
        {
            using (var scope = factory.Services.CreateScope())
            {
                ...
                var secondPerson = entities.FirstOrDefault(p => p.Id == 2);
        ...
        public async Task GetFirstIncluding_WithIdInInput_ShouldReturnEntitiesWithIncludedProperties()
        {
            using (var scope = factory.Services.CreateScope())
            {
                ...
                var entity = await repository.GetFirstIncluding(1, p => p.Tasks);
        ...
        public async Task GetFirstIncluding_WithFuncInInput_ShouldReturnEntitiesWithIncludedProperties()
        {
            using (var scope = factory.Services.CreateScope())
            {
                ...
                var entity = await repository.GetFirstIncluding(x => x.Id == 1, p => p.Tasks);
        ...
        public async Task DeleteWhere_ShouldDeleteEntity()
        {
            using (var scope = factory.Services.CreateScope())
            {
                var context = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
                var repository = new EfRepository<Person, int>(context);
                await repository.DeleteWhere(x => x.Id == 2);
                await context.SaveChangesAsync();

                var persons = await repository.GetAllList();

                Assert.Single(persons);
                Assert.DoesNotContain(persons, p => p.Id == 2);            }
